...
local bossesnames = {
    ...
    mock_dragonfly = "Wilting Dragonfly",
    mothergoose = "The Mother Goose",
    moonmaw_dragonfly = "Moonmaw Dragonfly",
    hoodedwidow = "The Hooded Widow",
    eyeofterror = "The Eye of Terror",
    dragonfly = "Mother Dragonfly",
    moose = "The Moose/Goose",
    bearger = "Dormant Bearger",
    mutatedbearger = "Armored Bearger",
    enraged_klaus = "Vengeful Klaus ⚠",
    mutateddeerclops = "Crystal Deerclops",
    twinofterror2 = "Hungry Spazmatism",
    antlion = "Desert Antlion",
    toadstool_dark = "Misery Toadstool ⚠",
    enraged_dragonfly = "Burning Dragonfly ⚠",
    twinofterror1 = "Seeker Retinazor",
    stalker_atrium = "Ancient Fuelweaver",
    sharkboi = "Defiant Frostjaw",
    mutatedwarg = "Possessed Warg",
    shadow_knight = "Shadow Knight",
    shadow_bishop = "Shadow Bishop",
    beequeen = "Royal Bee Queen",
    crabking = "Mighty Crab King",
    deerclops = "Chilling Deerclops",
    daywalker = "Nightmare Werepig",
    minotaur = "Ancient Guardian",
    daywalker2 = "Scrappy Werepig",
    malbatross = "Flying Malbatross",
    shadow_rook = "Shadow Rook",
    klaus = "Wicked Klaus",
    toadstool = "Grotto Toadstool",
...
function HandleShardFunction(key, value)
    ...
    elseif key == "clientchatmessage" then
        if value.userid then
            local json = GLOBAL.json.encode({ type = value.type, message = value.message, name = value.name or nil, event = value.event or nil })
            if json then
...
function ExecuteOnAllShards(key, value, onlymaster)
...
function HoundEvent(inst, event)
    inst:DoTaskInTime(math.random(30, 90), function(inst)
        ...
        local name = target:GetDisplayName() or target.name or "alguém"
        ExecuteOnAllShards("clientchatmessage", { type = "bernie", message = "Sentiram o cheiro de " .. name .. "!" }, true)
...
function ColdSnapEvent(inst, event)
    for _, player in ipairs(GLOBAL.AllPlayers) do
        if player.components.temperature ~= nil then
            player.components.temperature:DoDelta(-1)
        end
    end
end
...
function CallCustomEvent(inst, event)
    ...
    if event.currentloop == 0 and event.messagestart ~= nil then
        ExecuteOnAllShards("clientchatmessage", event.messagestart, true)
        for _, player in ipairs(GLOBAL.AllPlayers) do
    ...
    if event.currentloop >= event.loop then
        if event.messageend then
            ExecuteOnAllShards("clientchatmessage", event.messageend, true)
            for _, player in ipairs(GLOBAL.AllPlayers) do
                if player ~= nil and player:IsValid() and player.SoundEmitter ~= nil then
    ...
    inst:DoTaskInTime(12, function()
        CallCustomEvent(inst, event)
    end)
end

local customevents = {
    houndattack = {
        messagestart = { type = "bernie", message = "Feras selvagens estão à espreita!", event = "blink" },
        name = "houndwave",
        loop = 0,
        currentloop = 0,
        func = function(inst, event) HoundEvent(inst, event) end
    },
    heatwave = {
        messagestart = { type = "bernie", message = "Hoje o sol queima como o inferno!", event = "blink" },
        name = "heatwave",
        loop = 30,
        currentloop = 0,
        messageend = { type = "bernie", message = "O calor se dissipa..." },
        func = function(inst, event) HeatWaveEvent(inst, event) end
    },
    coldsnap = {
        messagestart = { type = "bernie", message = "Uma frente fria se aproxima...", event = "blink" },
        name = "coldsnap",
        loop = 30,
        currentloop = 0,
        messageend = { type = "bernie", message = "Os ventos gelados se foram!" },
        func = function(inst, event) ColdSnapEvent(inst, event) end
    },
    rain = {
        messagestart = { type = "bernie", message = "Nuvens de chuva sobrevoam o céu!", event = "blink" },
        name = "rain",
        loop = 0,
        currentloop = 0,
...
local function RollCustomEvent(inst)
    local rate = debugrate or 0.05
    if math.random() > debugrate then return end
    local keys = {}
    ...
    local event = customevents[randomkey]
    if event ~= nil then CallCustomEvent(inst, event) end
end
...
function PigmanBuildHome(inst)
    inst:WatchWorldState("phase", function(inst, phase)
        if phase == "dusk" then
            local HOUSELESS_PIG_HOUSE_CHANCE = 1 or 0.05
            ...
            local function GivePigNewHouse(inst)
                ...
                if math.random() > HOUSELESS_PIG_HOUSE_CHANCE then return end
...
local propaganda = {
    ...
    "Tem ideias para o servidor? discord.gg/37yfuWjyj7"
}
...
AddPrefabPostInit("world", function(inst)
    ...
    inst:DoPeriodicTask(60 * 60, function()
        ...
        ExecuteOnAllShards("clientchatmessage", data, true)
    end)
    inst:WatchWorldState("phase", function(inst, phase)
        if phase == "day" then
            local dice = math.random(12, 36)
            inst:DoTaskInTime(dice, function()
                RollCustomEvent(inst)
            end)
        end
    end)
end)
...
function Combat:GetAttacked(attacker, damage, weapon, stimuli, spdamage)
    ...
    -- if it's abigail, takes 50% less damage if attacker is an epic.
    if inst.prefab == "abigail" and attacker ~= nil and attacker:HasTag("epic") then damage = damage * 0.5 end

    -- if its a beefalo, takes 90% of the damage, and 10% goes to the rider
    if inst.prefab == "beefalo" and damage ~= nil and damage > 0 and inst.components.rideable ~= nil then
        local rider = inst.components.rideable:GetRider()
        if rider ~= nil then
            local rider_damage = damage * 0.10
            damage = damage * 0.90
            DamageRider(rider, rider_damage)
...
local function OnComponentEater(self)
    ...
    function self:Eat(food)
        ...
        if foodlist[food.prefab] and self.inst then
            ...
            if lastalert == "eat" .. doer .. victim then return end
            local userid = self.inst.userid
            lastalert = "eat" .. doer .. victim
            ExecuteOnAllShards("clientchatmessage", { type = "server", message = doer .. " está comendo " .. victim .. "...", whisper = false })
            local jsonEncoded = GLOBAL.json.encode({ key = "eat", doer = doer, userid = userid, victim = victim })
            SendRequest(jsonEncoded)
        end
        return result
    end
    ...
    function self:Ignite(immediate, source, ...)
        ...
        if self.inst and self.inst.prefab and source then
            ...
            if player and player:HasTag("player") then
                ...
                if lastalert == "burn" .. doer .. victim then return end
                ...
                lastalert = "burn" .. doer .. victim
                local jsonEncoded = GLOBAL.json.encode({ key = "burn", doer = doer, userid = userid, victim = victim, structure = structure })
                SendRequest(jsonEncoded)
            else
            end
        end
