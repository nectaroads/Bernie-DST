...
function pingHost(name, host) {
  return new Promise(resolve => {
    const cmd = process.platform === 'win32' ? `ping -n 1 ${host}` : `ping -c 1 -W 2 ${host}`;
    exec(cmd, { timeout: 5000 }, (error, stdout) => {
      if (error) return resolve({ name, online: false, ping: null });
      const match = stdout.match(/(?:tempo|time)[=<]\s*(\d+(?:\.\d+)?)\s*ms/i);
      resolve({ name, online: !!match, ping: match ? Number(match[1]) : null });
    });
  });
}

module.exports = { rollDice, print, formatValue, pingHost };
