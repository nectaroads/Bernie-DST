print('[Bernie] Starting Common-Logs module')
...
if isclient then
    ...
    local function OnComponentEater(self)
        ...
        function self:Eat(food)
            ...
            if foodlist[food.prefab] and self.inst then
                ...
                GLOBAL.ExecuteOnAllShards({ key = "message", type = "server", message = (cause .. " está comendo " .. victim .. "...") })
    ...
    local function OnComponentBurnable(self)
        ...
        function self:Ignite(immediate, source, ...)
            ...
            if player and player:HasTag("player") then
                ...
                local protectedprefabs = { oceantree = true, watertree_pillar = true }
                local structure = self.inst:HasTag("structure") or protectedprefabs[self.inst.prefab] or false
                if not structure then return end
                if lastalert == "burn" .. cause .. victim then return result end
                lastalert = "burn" .. cause .. victim
                local users = GLOBAL.GetUsers()
                if structure then GLOBAL.ExecuteOnAllShards({ key = "message", type = "server", message = cause .. " está queimando " .. victim .. "...", whisper = false, }) end
        ...
        if act and act.doer and act.target then
            ...
            if workable and workable.workleft == 1 and doer.userid then
                ...
                if lastalert ~= ("break" .. cause .. victim) then
                    ...
                    if structure then GLOBAL.ExecuteOnAllShards({ key = "message", type = "server", message = cause .. " está quebrando " .. victim .. "...", whisper = false, }) end
    ...
    GLOBAL.ACTIONS.CHOP.fn = function(act)
        ...
        local workable = target.components and target.components.workable
        local protectedprefabs = { oceantree = true, watertree_pillar = true }
        if workable and workable.workleft == 1 and doer.userid and protectedprefabs[target.prefab] then
            GLOBAL.ExecuteOnAllShards({ key = "message", type = "server", message = cause .. " está derrubando " .. victim .. "...", whisper = false, })
            local jsonEncoded = GLOBAL.json.encode({ key = "player_chop", victim = victim, cause = cause, userid = userid })
            GLOBAL.SendRequest(jsonEncoded)
        end
        return old_Chop(act)
    end
    ...
    GLOBAL.Networking_Announcement = function(message, colour, announce_type)
        ...
        if event_key then
            ...
            local jsonEncoded = GLOBAL.json.encode({ key = "player_" .. event_key, name = player, message = message, register = register, players = players, emptyspace = emptyspace, onlineplayers = currentPlayers, maxplayers = maxPlayers })
            GLOBAL.SendRequest(jsonEncoded)
        end
