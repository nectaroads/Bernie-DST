...
local function GetInventoryOwner(inst)
    return inst.components.inventoryitem ~= nil and inst.components.inventoryitem.owner or nil
end

local function GetInventoryInsulationChance(inst, insulationtype)
    local owner = GetInventoryOwner(inst)
    if owner == nil or owner.components.inventory == nil then return 0 end
    local insulation = 0
    for _, item in pairs(owner.components.inventory.equipslots) do
...
local function fn()
    ...
    local _SetTemperature = inst.components.temperature.SetTemperature

    inst.components.temperature.SetTemperature = function(self, value, ...)
        local stone = self.inst
        local current = self:GetCurrent()
        local owner = GetInventoryOwner(stone)
        if owner ~= nil and value ~= current then
            local ambient = GetLocalTemperature(stone)
            local insulationtype = nil
            if current > ambient and value < current then
                insulationtype = SEASONS.WINTER
            elseif current < ambient and value > current then
                insulationtype = SEASONS.SUMMER
            end
            if insulationtype ~= nil then
                local chance = GetInventoryInsulationChance(stone, insulationtype)
