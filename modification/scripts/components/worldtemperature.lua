...
return Class(function(self, inst)
    ...
    local MIN_TEMPERATURE = -32.5
    local MAX_TEMPERATURE = 123.5
    ...
    function inst:SetCustomTemp(value)
        if _ismastersim then
            _extratemperature:set(value or 0)
        end
    end
    ...
    local function CalculateTemperature()
        ...
        return (((temperaturenoise + _seasontemperature + _phasetemperature + (_extratemperature:value() or 0)) - _globaltemperaturelocus) * _globaltemperaturemult) + _globaltemperaturelocus
    end
